 ...
 * @param {number=} start
 * @yields {string|jsx.JSX.Element} Spans with hlClass for CSS class for highlighted text, in between strings for unhighlighted text
 */
export function *highlightedElements(content, toHighlight, hlClass, start) {
    if (toHighlight.length < 1)
        ...
        yield* unfold(
             ...
             * @param {number} start
             * @returns {[(string|jsx.JSX.Element)[], true] | [(string|jsx.JSX.Element)[], false, string, string, number]}
             */
...
export default props =>
    <Fragment>
        {[...highlightedElements(props.Content,
                                 props.Search,
